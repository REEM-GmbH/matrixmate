			...
			foreach ($settings as $contextKey => $contextSettings) {
				...
				foreach ($contextArray as $context) {
					...
					if (str_starts_with($context, 'entryType:')) {
						...
						if ($entryTypeHandle && $entryTypes = Craft::$app->getEntries()->getEntryTypeByHandle($entryTypeHandle)) {
						...
						if ($sectionHandle && $section = Craft::$app->getEntries()->getSectionByHandle($sectionHandle)) {
